        ...
        protected override void CreateChildControls()
        {
            if (cItem.ProductID > 0)
            {
                ...
                {                   
                    ...
                    lblpricewithBluBucksUsed.Text = cItem.pricewithBluBuksUsed.ToString();
                    lblpricewithCategoryFundUsed.Text = "$" + cItem.pricewithategoryFundUsed;
                ...
                if (this.DisplayMode != CartDisplayMode.MiniCart)
                {
                    ...
                    else
                    {
                        ...
                        lblSubTotal.Text = "<span id='" + "spregularprice_" + cItem.ShoppingCartRecordID.ToString() + "' class='hide-element spregularprice'><b>Regular Price: </b>" + string.Format(CultureInfo.GetCultureInfo("en-US"), AppLogic.AppConfig("CurrencyFormat"), RegularPrices) + "</span>";
                        if (Convert.ToDecimal(lblpricewithCategoryFundUsed.Text.Replace("$", "")) > 0)
                            lblSubTotal.Text += "<span id='" + "spfunddiscountprice1_" + cItem.ShoppingCartRecordID.ToString() + "' class='hide-element spfunddiscountprice'>" + cItem.FundName + string.Format(CultureInfo.GetCultureInfo("en-US"), AppLogic.AppConfig("CurrencyFormat"), Convert.ToDecimal(lblpricewithCategoryFundUsed.Text.Replace("$", ""))) + "</span>";
                        if (Convert.ToDecimal(lblpricewithBluBucksUsed.Text.Replace("$", "")) > 0)
                            lblSubTotal.Text += "<span id='" + "spblubucksprice1_" + cItem.ShoppingCartRecordID.ToString() + "' class='hide-element spblubucksprice'>BLU Bucks used: " + string.Format(CultureInfo.GetCultureInfo("en-US"), AppLogic.AppConfig("CurrencyFormat"), Convert.ToDecimal(lblpricewithBluBucksUsed.Text)).Replace("$", "") + "" + "</span>";
