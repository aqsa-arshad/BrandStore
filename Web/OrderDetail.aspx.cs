...
public partial class OrderDetail : SkinBase
{
        ...
        using (var conn = DB.dbConn())
        {
            ...
            using (var cmd = new SqlCommand("aspdnsf_GetOrderDetail", conn))
            {
                ...
                if (reader.Read())
                {
                    ...
                    //Billing Address
                    lblBAFullName.Text = reader["BillingFirstName"].ToString() + ' ' + reader["BillingLastName"].ToString();
                    ...
                    lblBACityStateZip.Text = reader["BillingCity"] + ", " + reader["BillingState"] + ' ' + reader["BillingZip"];
                    lblBACountry.Text = reader["BillingCountry"].ToString();
                    lblBAPhone.Text = reader["BillingPhone"].ToString();
                    //Shipping Address
                    lblSAFullName.Text = reader["ShippingFirstName"].ToString() + ' ' + reader["ShippingLastName"].ToString();
                    ...
                    lblSACityStateZip.Text = reader["ShippingCity"] + ", " + reader["ShippingState"] + ' ' + reader["ShippingZip"];
                    lblSACountry.Text = reader["ShippingCountry"].ToString();
                    lblSAPhone.Text = reader["ShippingPhone"].ToString();
                    //Payment Methods
                    lblPMCardInfo.Text = reader["CardType"].ToString() + ' ' + (string.Concat("*********", reader["CardNumber"].ToString()));
                    lblPMExpireDate.Text = "Expires: " + reader["CardExpirationMonth"].ToString() + '/' + reader["CardExpirationYear"].ToString();
    ...
    public string GetReorder()
    {
        ...
        {            
            return "<a class=\"btn btn-primary btn-half\" role=\"button\" href=\"javascript:ReOrder(" + orderNumber + ");\">" + "Print Receipt" + "</a>";
        }
        else
        {
            return String.Empty;
