...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="Entity">
		<sitemap>
			<loc>
				<xsl:value-of select="$StoreLoc"/>mobilegoogleentity.aspx?entityname=<xsl:value-of select="$entity"/>&amp;entityid=<xsl:value-of select="EntityID"/>
			</loc>
      <!--<xsl:text disable-output-escaping="yes">&lt;mobile:mobile/&gt;</xsl:text>-->
		</sitemap>
    <xsl:text>&#13;</xsl:text>
