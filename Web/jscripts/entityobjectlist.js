...
aspdnsf.Controls.EntityObjectList = function(txtId, radWinId) {
    this.txt = $get(txtId);
    this.radWin = $find(radWinId);
    this.posFixed = false;
...
aspdnsf.Controls.EntityObjectList.prototype = {

    get_txtId : function() {
        return this.txt.id;
    },
    
    set_txtValue : function(value) {
    ...
    adjustOffset : function() {
        ...
        var fixPos = function(){
            if(!this.posFixed) {
                var rbound = this.radWin.getWindowBounds();
                var tbound = WebForm_GetElementPosition(this.txt);
                rbound.y += tbound.height;
                this.radWin.moveTo(rbound.x, rbound.y);
                this.posFixed = true;                            
            }
        };
        
        var del = Function.createDelegate(this, fixPos);    
        this.radWin.add_show( del )
    },
    
    closeRadwin : function() {
        this.radWin.close();
    }
    
}
aspdnsf.Controls.EntityObjectList.registerClass('aspdnsf.Controls.EntityObjectList');
...
aspdnsf.Controls.EntityObjectListManager = new function() {
    this.ctrls = new Array();
    
    this.register = function(ctrl){ this.ctrls.push(ctrl); };
    
    this.pushData = function(txtId, txtValue){
    
        var callBack = function(ctrl) {
            if(ctrl.get_txtId() == txtId) {
                var val = HtmlDecode(txtValue);
                ctrl.set_txtValue(val);
                ctrl.closeRadwin();
            }
        }
        
        $foreach(this.ctrls, callBack);
        
    };    
}

// register alias for root reference
window.EntityObjectListManager = aspdnsf.Controls.EntityObjectListManager;
