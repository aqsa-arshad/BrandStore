$(document).ready(function () {
    ...
    aspdnsf.Controls.AddToCartForm.prototype = {
        ...
        getElementSelectedIndex: function (name) {
            var index = $("select[name='" + name + '_' + this.getProductId() + '_' + this.getVariantId() + "'] option:selected").index();
            return index;
        },
        ...
        getStockOnHand: function () {
            try {
                var stock = -1;
                if ($(".inventory-table-wrap > .in-stock-hint").length) {
                    stock = $(".inventory-table-wrap > .in-stock-hint").text().replace("In stock:", "");
                }
                if ($(".inventory-table-wrap > .out-stock-hint").length) {
                    stock = 0;
                }
        ...
        getStockOnHandVariant: function () {
            try {
                var i = this.getElementSelectedIndex('Color');
                var j = this.getElementSelectedIndex('Size');
                var stock = $("table.inventory-table").find("tr").eq(i).find("td").eq(j).text();
                return parseInt(stock);
            }
            catch (err) {
                return -1;
        ...
        onButtonClick: function (cartType) {
            ...
            var stock = this.getStockOnHandVariant();
            if (!isNaN(stock)) {
                if (stock != -1) {
                    if (qty > stock) {
                        alert("Your order may be delayed because you have selected more than what is currently in stock. Please select a smaller quantity in your cart if you want to avoid any shipping delays.");
