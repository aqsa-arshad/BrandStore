						...
						else
						{
							...
							using (SqlConnection dbconn4 = DB.dbConn())
							{
								...
								using (IDataReader rs2 = DB.GetRS("Select * from orders  with (NOLOCK)  where ordernumber=" + OrderNumber.ToString(), dbconn4))
								{
                                    ...
                                    // Custom addition: Export XML for ProfitMaker
                                    sb.Append("<tr><td align=\"right\" valign=\"middle\">" + AppLogic.GetString("admin.orderframe.Export", SkinID, LocaleSetting) + "&nbsp;</td><td align=\"left\" valign=\"middle\">");
                                    if (DB.RSFieldBool(rs2, "Exported"))
                                    {
                                        sb.Append(AppLogic.GetString("admin.common.Yes", SkinID, LocaleSetting) + "&nbsp;<input  class=\"normalButtons\" type=\"button\" value=\"" + AppLogic.GetString("admin.orderframe.ReDownloadExportXML", SkinID, LocaleSetting) + "\" name=\"DownloadExportXML" + DB.RSFieldInt(rs2, "OrderNumber").ToString() + "\" onClick=\"DownloadExportXML(OrderDetailForm," + DB.RSFieldInt(rs2, "OrderNumber").ToString() + ");\">");
                                    }
                                    else
                                    {
                                        ...
                                        {
                                            sb.Append("<input  class=\"normalButtons\" type=\"button\" value=\"" + AppLogic.GetString("admin.orderframe.DownloadExportXML", SkinID, LocaleSetting) + "\" name=\"DownloadExportXML" + DB.RSFieldInt(rs2, "OrderNumber").ToString() + "\" onClick=\"DownloadExportXML(OrderDetailForm," + DB.RSFieldInt(rs2, "OrderNumber").ToString() + ");\">");
                                        }
                                    }
                                    sb.Append("</td></tr>\n");
                                    ...
                                    if (SubmitAction == "DOWNLOADEXPORTXML")
                                    {
                                        sb.Append("<iframe id=\"exportXml\" style=\"position:absolute;top:-9999px;\" src=\"export.aspx?ordernumber=" + DB.RSFieldInt(rs2, "OrderNumber").ToString() + "\"></iframe>");
                                    }
                                    // End custom addition: Export XML for ProfitMaker
