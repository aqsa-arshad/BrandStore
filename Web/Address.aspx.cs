...
namespace AspDotNetStorefront
{
    public partial class _Address : SkinBase
    {
        ...
        {
            SectionTitle = AppLogic.GetString("Header.AddressBook", SkinID, ThisCustomer.LocaleSetting);
        ...
        private void LoadData()
        {
			...
			foreach (Address address in GetAddresses())
			{
                ...
                if (!(new POBoxAddressValidator()).IsValid(address) && (address.AddressType == AddressTypes.Shipping) && (AppLogic.AppConfigBool("DisallowShippingToPOBoxes")))
                {
                ...
                if (ctrlAddress != null)
                {
                    ...
                    if (AppLogic.AppConfig("VerifyAddressesProvider").Length > 0)
                    {
                        ...
                        if (validateResult != AppLogic.ro_OK)
                        {
							...
							//Update the entered address to what was returned.  This will be the original address if the verification process failed somehow.
                        ...
                        using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                        {
                            ...
                            using (IDataReader rs = DB.GetRS(sSqlPO.ToString(), con))
                            {
                                while (rs.Read())
                                {
									...
									isPOBoxAddress = !(new POBoxAddressValidator()).IsValid(tempAddress1);
                                }
                            }
                        }
                    }
        ...
        protected void dlAddress_ItemDataBound(object sender, DataListItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.EditItem)
            {
                ...
                CreditCardPanel ctrlCreditCard = e.Item.FindControl("ctrlCreditCard") as CreditCardPanel;
                Echeck ctrlECheck = e.Item.FindControl("ctrlECheck") as Echeck;
                int addressId = Convert.ToInt32(DataBinder.Eval(e.Item.DataItem, "AddressID"));
                PopulateAddressControlValues(ctrlAddress, ctrlCreditCard, ctrlECheck, e.Item.ItemIndex, addressId);
                ...
                e.Item.ItemType == ListItemType.AlternatingItem))
            {
                ...
                if (itemAddressID == primaryID)
                {
                    ...
                    AddressHTML.CssClass = "address-wrap";
                    if (AddressMode == AddressTypes.Billing)
                    {
